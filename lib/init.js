...
exports.init = async function (args, sveEndpoint, geeEndpoint) {
  ...
  //todo question에 anwser 입력 안했으면 다시 물어보는 로직 추가.

  //Todo npm publish도 ignore 설정 가능한 지 확인. (불필요함?)

  //todo 이미 env가 다른 이름으로 존재해도 asterisc로 전부 삭제하고, 생성하도록 하는게 더 좋다.(지금은 동일 파일이름만 삭제함)
